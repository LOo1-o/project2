...
def normalize_number_string(value: str, force_decimal: bool = False) -> str:
    ...
    # Удаляем пробелы из чисел. Не только обычный пробел: в выгрузках
    # Росстата разделителем тысяч обычно стоит НЕРАЗРЫВНЫЙ пробел, а он
    # раньше оставался в значении. Такое число не подходило под шаблон
    # целого числа, поэтому мимо него молча проходило и выравнивание
    # десятичных знаков (force_decimal).
    for space in (" ", "\xa0", " ", " ", " ", "\t"):
